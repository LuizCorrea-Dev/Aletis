...
import { X, Hash, Loader2, Send, Upload } from "lucide-react";
...
export function CreatePostModal({ onClose, onSuccess, communityId }: CreatePostModalProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (mediaPreview) formData.append("mediaUrl", mediaPreview);
      ...
      if (result?.success) {
        ...
        setError(result?.message || "Erro ao publicar.");
      }
    } catch {
        ...
        <div className="bg-[#1e293b] w-full max-w-xl rounded-t-3xl md:rounded-3xl border-t md:border border-slate-700 shadow-2xl flex flex-col max-h-[90vh] animate-in slide-in-from-bottom-4 md:slide-in-from-bottom-0 duration-300">
          ...
          <div className="flex items-center justify-between px-6 py-5 border-b border-slate-700/60">
            <div>
              ...
              <p className="text-xs text-slate-400">Compartilhe o que está sentindo</p>
            </div>
            <button
              onClick={onClose}
          ...
          <div className="px-6 pb-6 pt-4 border-t border-slate-700/60">
            <button
              ...
              className="w-full bg-[#50c878] hover:bg-[#50c878]/90 text-[#1e293b] font-extrabold py-4 rounded-2xl shadow-lg transition-all active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 text-sm"
            >
              {isSubmitting ? (
