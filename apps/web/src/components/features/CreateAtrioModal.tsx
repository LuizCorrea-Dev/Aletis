...
export function CreateAtrioModal({ onClose, onSuccess }: CreateAtrioModalProps) {
  ...
  const [mediaPreview, setMediaPreview] = useState<{ url: string; type: "image" | "video" | "pdf"; name: string; sizeMB: number } | null>(null);
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (mimeType.startsWith("image/") || [".png", ".jpg", ".jpeg", ".webp", ".gif", ".svg"].includes(extension)) {
      ...
      setMediaPreview({ url, type: "image", name: filename, sizeMB: fileSizeMB });
    }
    // 2. Vídeos (MP4, WEBM)
      ...
      if (![".mp4", ".webm"].includes(extension) && !mimeType.includes("mp4") && !mimeType.includes("webm")) {
        setFileError({
          ...
          message: `O vídeo "${filename}" utiliza o formato "${extension}". Apenas vídeos em MP4 ou WEBM são reproduzidos nativamente no Átrio.`,
      ...
      const url = await readFileAsDataURL(file);
      setMediaPreview({ url, type: "video", name: filename, sizeMB: fileSizeMB });
    }
    // 3. Documentos PDF
      ...
      const url = await readFileAsDataURL(file);
      setMediaPreview({ url, type: "pdf", name: filename, sizeMB: fileSizeMB });
    }
    // 4. Formatos Não Suportados
      ...
      setFileError({
        ...
        message: `O arquivo "${filename}" (extensão ${extension}) não é suportado no Átrio.`,
  ...
  const handleSubmit = async () => {
    ...
    if (!mediaPreview) {
      setError("Adicione um arquivo de mídia (imagem, vídeo ou PDF) para publicar no Átrio.");
      return;
    }
    ...
    try {
      ...
      formData.append("url", mediaPreview.url);
      ...
      if (res.success) {
        ...
        const msg = res.message || "Erro ao publicar no Átrio.";
        if (msg.includes("Failed to find Server Action")) {
          ...
          msg.includes("atrio_items_user_id_fkey") ||
          msg.includes("23503") ||
      ...
      if (errStr.includes("Failed to find Server Action")) {
        ...
        setFileError({
          title: "Sessão de Conta Não Localizada",
          filename: "Perfil de Usuário",
          message: "Sua conta de usuário não foi encontrada no banco de dados ativo.",
          recommendation: "Faça login novamente no Aletis para revalidar a sua sessão.",
          actionButton: {
            label: "Ir para a Tela de Login",
            onClick: () => {
        ...
        setError("Erro inesperado ao publicar no Átrio.");
      }
    } finally {
  ...
  return (
    <div className="fixed inset-0 z-40 bg-black/85 backdrop-blur-md flex items-center justify-center pt-16 sm:pt-20 pb-20 sm:pb-24 px-3 sm:px-4 xl:p-4 animate-in fade-in duration-200">
      <div
        className="bg-[#1e293b] w-full max-w-lg max-h-[calc(100vh-9.5rem)] xl:max-h-[85vh] rounded-2xl sm:rounded-3xl border border-slate-700 shadow-2xl overflow-hidden flex flex-col animate-in zoom-in-95 duration-200 my-auto"
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-slate-700/60 bg-slate-900/40">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">Compartilhe fotos, vídeos ou documentos contemplativos</p>
            </div>
          </div>
          <button
            onClick={onClose}
            className="w-9 h-9 rounded-full text-slate-400 hover:text-white hover:bg-slate-800 flex items-center justify-center transition-all cursor-pointer shrink-0 border border-slate-700/60"
            aria-label="Fechar"
          >
            <X size={18} />
          </button>
        ...
        {/* Form Body */}
        <div className="p-4 sm:p-6 space-y-4 overflow-y-auto max-h-[calc(100vh-10rem)] xl:max-h-[75vh]">
          <div>
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Escreva a mensagem ou sentimento por trás desta publicação..."
              rows={4}
          ...
          <div>
            <label className="text-[10px] font-bold uppercase tracking-wider text-slate-400 block mb-1.5">
              Mídia da Obra (Fotos, Vídeos, PDFs) *
            </label>
            {mediaPreview ? (
              <div className="relative rounded-2xl overflow-hidden border border-slate-700 bg-slate-900">
                {mediaPreview.type === "image" && (
                  <img src={mediaPreview.url} alt="Preview" className="w-full h-48 object-cover" />
                )}
                {mediaPreview.type === "video" && (
                  <video src={mediaPreview.url} controls className="w-full max-h-48 rounded-2xl bg-black" />
                )}
                {mediaPreview.type === "pdf" && (
                  <div className="p-6 bg-slate-800 flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-bold text-white truncate">{mediaPreview.name}</p>
                      <p className="text-[10px] text-slate-400">Documento PDF ({mediaPreview.sizeMB.toFixed(1)} MB)</p>
                    </div>
                  </div>
                )}
                <button
              ...
              <button
                ...
                className="w-full py-6 border border-dashed border-slate-700 hover:border-[#2dd4bf] rounded-2xl flex flex-col items-center justify-center gap-2 text-slate-400 hover:text-white transition-all bg-slate-900/40 cursor-pointer"
              >
                ...
                <span className="text-xs font-bold text-slate-200">
                  Enviar Foto (15MB), Vídeo (150MB) ou PDF (25MB)
                </span>
              </button>
            )}
            <input
        ...
        <div className="p-5 border-t border-slate-700/60 flex justify-end gap-3 bg-slate-900/40">
          ...
          <button
            ...
            disabled={isSubmitting || !title.trim() || !mediaPreview}
